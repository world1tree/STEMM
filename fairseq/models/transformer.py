...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if self.encoder_layerdrop > 0.0:
            self.transformer_translation_layers = LayerDropModuleList(p=self.encoder_layerdrop)
        else:
            self.transformer_translation_layers = nn.ModuleList([])
        self.transformer_translation_layers.extend(
            [self.build_encoder_layer(args) for i in range(args.encoder_layers)]
        )
        self.num_layers = len(self.transformer_translation_layers)

        if args.encoder_normalize_before:
            self.translation_layer_norm = LayerNorm(embed_dim, export=export)
        else:
    ...
    def forward_embedding(
        ...
        if self.embed_source_positions is not None:
            x = embed + self.embed_source_positions(src_tokens)
    ...
    def forward_scriptable(
        ...
        if self.translation_layer_norm is not None:
            x = self.translation_layer_norm(x)
    ...
    def max_positions(self):
        ...
        if self.embed_source_positions is None:
            return self.max_source_positions
        return min(self.max_source_positions, self.embed_source_positions.max_positions)

    def upgrade_state_dict_named(self, state_dict, name):
        """Upgrade a (possibly old) state dict for new versions of fairseq."""
        if isinstance(self.embed_source_positions, SinusoidalPositionalEmbedding):
            weights_key = "{}.embed_source_positions.weights".format(name)
            if weights_key in state_dict:
                print("deleting {0}".format(weights_key))
                del state_dict[weights_key]
            state_dict[
                "{}.embed_source_positions._float_tensor".format(name)
            ] = torch.FloatTensor(1)
        for i in range(self.num_layers):
            # update layer norms
            self.transformer_translation_layers[i].upgrade_state_dict_named(
                state_dict, "{}.transformer_translation_layers.{}".format(name, i)
            )
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, cross_attn_hidden = layer(
                x,
                enc,
                padding_mask,
                incremental_state,
            ...
            inner_states.append(cross_attn_hidden)
        ...
        return x, {"attn": [attn], "inner_states": inner_states, "final_states": final_states}
