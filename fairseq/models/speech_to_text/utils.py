...
def get_prob(num_updates, mixup_arguments, is_training):
    if not is_training:
        return 1.0
    args = mixup_arguments.split(',')
    strategy = args[0]
    if strategy == "fix":
        mix_step, mix_prob = args[1:]
        mix_step = int(mix_step)
        mix_prob = float(mix_prob)
        return mix_prob if num_updates <= mix_step else 0.0
    else:
        raise NotImplementedError("Unimplemented mixup strategy")
...
def save_to_dict(encoder_out, encoder_padding_mask):
    return {
